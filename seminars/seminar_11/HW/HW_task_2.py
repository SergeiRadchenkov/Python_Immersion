...
в архив (списки archive_text и archive_number) для уже существующего экземпляра. Если экземпляр еще не существует,
...
class Archive:
    ...
    def __new__(cls, letter: str, number: int):
        instance = super().__new__(cls)
        instance.letter = letter
        instance.number = number
        cls.letters.append(letter)
        cls.numbers.append(number)
        instance.archive_text = cls.letters.copy()
        instance.archive_number = cls.numbers.copy()
        return instance

    def __str__(self):
        return f'Text is {self.letter} and number is {self.number}. Also {self.archive_text} and {self.archive_number}'
